...
from utils.helpers import replace_chords_with_underscore
...
async def load_lyrics(page, id):
    try:
        ...
        data = {
            ...
            "image": "",
            "chord": [],
            "text": []  # Initialize an empty list to store lyrics text
        }
        ...
        for key, element in elements.items():
            if element:
                if key == "image":
                    data[key] = await page.evaluate('(element) => element.getAttribute("src")', element)
                else:
                    data[key] = await page.evaluate('(element) => element.textContent', element)

        if "chord" in data:
            data["chord"] = [i.replace('คอร์ด ', '') for i in data["chord"].split(', ')]
        else:
            data["chord"] = []
        
        # Process and add lyrics text if chords and title_elements exist
        if title_elements and data["chord"]:  # Check if chords_list is not empty
            for element in title_elements:
                title = await element.getProperty('textContent')
                title_text = await title.jsonValue()
                clean_text = replace_chords_with_underscore(title_text, data["chord"])
                data["text"].append(clean_text)  # Store cleaned lyrics text in the dictionary
        else:
            print("ไม่พบแท็กที่ต้องการ")
